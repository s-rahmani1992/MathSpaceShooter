...
public class MissileUp : MonoBehaviour {
    ...
    private void OnMouseUp()
    {
        ...
        if (upNum < dat.missileBuyIndexes[x.currentIndex - 3]) // selecting bought item
        {
            dat.currentMissiles[x.currentIndex - 3] = upNum;
            ...
            transform.parent.GetChild(up.GetChild(x.currentIndex).GetComponent<MissilePlacement>().missileNum).gameObject.SetActive(true);


            up.GetChild(x.currentIndex).GetComponent<MissilePlacement>().missileNum = upNum;
            gameObject.SetActive(false);
        }

        else if (upNum == dat.missileBuyIndexes[x.currentIndex - 3]) // buying the item
        {
            if (dat.coinCount >= price)
            {
                GetComponent<SpriteRenderer>().sprite = x.select;
                dat.missileBuyIndexes[x.currentIndex - 3]++;
                transform.parent.GetChild(dat.missileBuyIndexes[x.currentIndex - 3]).GetComponent<SpriteRenderer>().color = Color.white;
                dat.coinCount -= price;
